...
/**
 * PostMessageCameraListener — receives `memory-world:set-camera` messages
 * from a parent window (the lab iframe host) and smoothly transitions the
 * camera + OrbitControls target to the requested position.
 *
 * Must be rendered inside a Canvas that also has OrbitControls with a ref
 * passed via `controlsRef`.
 */
function PostMessageCameraListener({ controlsRef }) {
  const { camera } = useThree();
  ...
  useEffect(() => {
    function handleMessage(event) {
      ...
      if (!data || data.type !== 'memory-world:set-camera') return;

      const { position, target, fov } = data.camera || {};
      ...
      // Smooth lerp transition over ~40 frames (~0.67s at 60fps)
      ...
      const endFov = typeof fov === 'number' ? fov : camera.fov;
      ...
      function step() {
        ...
        if (t < 1) {
          ...
          animRef.current = null;
          // Notify parent that transition completed
          window.parent?.postMessage?.({
            type: 'memory-world:camera-ready',
  ...
  return (
    <>
      <OrbitControls
        ...
        screenSpacePanning
      />
      <PostMessageCameraListener controlsRef={controlsRef} />
    </>
  );
}
