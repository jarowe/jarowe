...
export default function MemoryWorldLab() {
  ...
  const sceneCam = useMemo(() => {
    // camera comes from meta.json top-level `camera` field, returned by lab API
    const cam = data?.scene?.camera ?? null;
    if (cam?.startPosition && cam?.startTarget) {
      return { startPosition: cam.startPosition, startTarget: cam.startTarget, fov: cam.fov ?? 48 };
    }
    return null;
  }, [data?.scene?.camera]);

  const sendCameraPreset = useCallback((presetId) => {
    if (!iframeRef.current?.contentWindow) return;

    // Build camera config from meta.json camera or fallback
    const cam = sceneCam ?? {
    ...
    const message = buildCameraPresetMessage(presetId, cam);
    if (!message) return;
  ...
  // Listen for camera-ready confirmation from the iframe
  useEffect(() => {
    function handleMessage(event) {
      if (event.data?.type === 'memory-world:camera-ready') {
  ...
  // Reset active preset when changing scene or source
  useEffect(() => {
    setActivePreset(null);
    setCameraTransitioning(false);
  }, [sceneId, effectiveSource]);
  ...
  useEffect(() => {
    function onKeyDown(event) {
      ...
      // Camera preset shortcuts: Shift+0 through Shift+6 → V0-V6
      if (event.shiftKey && /^[)!@#$%^]$/.test(event.key)) {
        event.preventDefault();
        const shiftMap = { ')': 'V0', '!': 'V1', '@': 'V2', '#': 'V3', $: 'V4', '%': 'V5', '^': 'V6' };
        const presetId = shiftMap[event.key];
        if (presetId) sendCameraPreset(presetId);
        return;
      }
  ...
  }, [candidates, effectiveSource, activeCandidateIndex, grade, label, notes, favorite, saving, sceneId, view, layoutMode, sendCameraPreset]);
        ...
        <div className="memory-world-lab__grid">
            ...
            <div className="memory-world-lab__camera-presets">
              ...
              {CAMERA_PRESETS.map((preset) => (
                <button
                  key={preset.id}
                  type="button"
                  className={`memory-world-lab__camera-preset ${activePreset === preset.id ? 'is-active' : ''} ${cameraTransitioning && activePreset === preset.id ? 'is-transitioning' : ''}`}
                  onClick={() => sendCameraPreset(preset.id)}
                  title={`${preset.name}: ${preset.description}\nEvaluate: ${preset.evaluate}\nShortcut: Shift+${preset.shortcut}`}
                  disabled={cameraTransitioning}
                >
                  <span className="memory-world-lab__camera-preset-id">{preset.id}</span>
                  <span className="memory-world-lab__camera-preset-name">{preset.name}</span>
                </button>
              ))}
            </div>
          ...
          <aside className={`memory-world-lab__inspector ${layoutMode === 'focus' ? 'memory-world-lab__overlay-panel' : ''} ${inspectorPanelOpen ? 'is-open' : ''}`}>
            ...
            <div className="memory-world-lab__detail-card">
              ...
              <div className="memory-world-lab__status-list">
                <span>1-0 grade</span>
                <span>Shift+0-6 camera presets V0-V6</span>
